...
2. スマホのChromeで上記URLにアクセス
3. 🎤 ボタンをタップして話す
4. 話し終わると自動でPCのクリップボードへ送信される
5. VS CodeのClaudeCodeチャット欄をクリックして `Ctrl+V` で貼り付け

### PCクリップボードをスマホへ取得

「📥 PCクリップボードを取得」ボタンをタップすると、PCのクリップボード内容がプレビュー領域に表示され、履歴にも追加されます。
...
**Step 2: プロファイルのインストール**

1. 設定アプリが開き「プロファイルがダウンロードされました」と表示される
2. 設定 → 一般 → VPNとデバイス管理 を開く
3. ダウンロードされたプロファイルをタップ
4. 右上の「インストール」をタップ
5. 警告画面で再度「インストール」をタップ
6. 「完了」をタップ
