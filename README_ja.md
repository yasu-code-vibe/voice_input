...
スマホ（Android/iPhone）のマイクから音声入力し、PCのクリップボードへ転送するツール。
...
> **初回アクセス時は端末ごとに設定が必要です。**
> - Android：[初回設定: Android](#初回設定-android) を参照してください
> - iPhone：[初回設定: iPhone](#初回設定-iphone) を参照してください
...
5. 貼り付けたいアプリのテキスト入力欄をクリックして `Ctrl+V` で貼り付け
...
- ブラウザのlocalStorageに保存（端末ごとに独立）。保存件数は設定画面から変更可能（デフォルト1000件、最大999999件）
